...
| D10 | **鸟种清单只收「磁盘上有自己目录」的鸟种**（至少有一张 2★ 及以上照片），与结果浏览器鸟种下拉、eBird 导出同一条线（2026-09-19 新增） | 此前报告收下每一条 `has_bird` 记录，同一批照片报告说 22 种、浏览器下拉说 20 种，多出来的是只有 0/1★ 照片、从没建过目录的鸟种。三处口径分裂时用户无从判断哪个数字是真的。收紧的只是「哪些鸟种上榜」——上榜鸟种的张数仍按它的全部照片计，因为在浏览器里按该鸟种筛选时低星照片同样会出现。封面取值范围跟着收紧（否则最醒目的位置会是清单里找不到的鸟），但一个鸟种都没上榜时退回全部有鸟照片，避免整批低星的批次拿到一张没有封面的空报告 |
...
- **鸟种榜**：按 `bird_species_cn/en` 分组 → 张数、最高星、`gbif_rarity_100` 分级、
  ...
  - **只收有目录的鸟种**（至少一张 2★ 及以上，见 D10）；上榜鸟种的张数仍按
    其全部照片计（2026-09-19 新增）
  - 这三项鸟种级附加数据在**改鸟种时必须按新鸟种的学名重查**（`core/
    species_extras.py`），查不到则写空。留着旧鸟种的值会让改正过的照片顶着
    错误的罕见度与濒危徽标，并按旧罕见度排在清单前列（2026-09-19 新增）
