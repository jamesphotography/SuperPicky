...
| D8 | **画廊按鸟种分组，每种最多 4 张**（1 张大代表作 + 3 张小图），区块按罕见度降序 | 读者的问题是「拍到了哪些鸟、每种长什么样」，不是「有多少张好片」；原「鸟种屏 + 精选画廊」两屏本就重复展示同一批代表作 |
...
同时展开为内存位图**。若把 lightbox 放大图也放进 DOM（12 鸟种 / 48 展示图 / 318 明细）：

```
放大图  48 张 × 1200×800 × 4B = 184.3 MB
展示图（封面/代表作/小图/明细，见 6.2）=  82.5 MB
                                        ─────────
                                          266.8 MB  ← 超出 iOS Safari 单页承受范围，有被系统终止的风险
```

**解法：把大图从 DOM 移入 JS 字符串数组。**

```html
<img src="data:...">                            <!-- 展示图 900/400px，会解码 -->
<script>const HD = ["data:...", ...]</script>   <!-- 放大图 1200px，仅字符串，不解码 -->
```
...
| 用途 | 长边 | JPEG 质量 | 典型数量 | 单张 | 小计 | 位置 |
|---|---|---|---|---|---|---|
| 封面 | 1800 | 82 | 1 | 400 KB | 0.4 MB | DOM |
| 鸟种代表作 | 900 | 80 | 12 | 110 KB | 1.3 MB | DOM |
| 鸟种小图 | 400 | 78 | 36 | 32 KB | 1.2 MB | DOM |
| lightbox 放大 | 1200 | 80 | 48 | 150 KB | 7.2 MB | **JS 数组** |
| 明细缩略图 | 160 | 72 | 318 | 12 KB | 3.8 MB | DOM |
| | | | | 小计 | 13.9 MB | |
| | | | | base64 ×1.33 | **≈ 18.5 MB** | |
...
D8 的「每种最多 4 张」已让展示图总量自然受控于鸟种数，但鸟种数本身仍需封顶
（一次跨国行程可能出现 40+ 种）：

- **鸟种区块封顶 30 种**，按罕见度降序取；其余归入区块末尾的「其他鸟种」纯文字列表
  （仅列名称与张数，不含图），页面标注「详展 30 / 共 N 种」
- **照片总数 > 600 时，明细表退化为纯文字**（去掉缩略图列），页面标注原因
- **导出弹窗先估算再确认**：按 6.2 的单张均值 × 实际数量估算，
  显示「预计生成约 N MB，用时约 M 秒」——让用户在按下之前就知道会发生什么
...
| 9 | 规模保护 | 40 个鸟种 → 只详展 30 种、其余入纯文字列表；700 张 → 明细表无缩略图列 |
...
18.5MB 文件体积与 82.5MB 常驻位图估算，并在真机上走一遍「存为 PDF」。
