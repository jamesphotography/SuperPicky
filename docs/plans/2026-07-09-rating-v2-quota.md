...
- [x] T7 验证(2026-07-12 完成):Test-Superpicky 实测通过——按鸟种配额数学逐组
      验算吻合(ceil(种内×20%),池302/3★93),硬门槛/锐度兜底/眼睛封顶正常,
      ARW 走 XMP 侧车(71=71)中文 EXIF 写读回正确,精选 Pick 正确,无异常回归;
      用户翻图确认 3★ 眼光无问题。观察点:①多鸟种目录实际 3★ 率高于配额
      面值(ceil+保底所致,ChangeLog 已注明);②低置信度识鸟参与配额分组,
      日志计数与文件夹归类口径不同(候选种 vs 其他鸟类)
